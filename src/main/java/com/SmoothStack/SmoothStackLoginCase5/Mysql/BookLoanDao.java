...
public class BookLoanDao implements IBookLoanDao {
	...
	public List<BookLoan> findAll(Connection conn) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				bookLoan = new BookLoan(book, libraryBranch, borrower);
	...
	public boolean addBookLoan(Connection conn, BookLoan bookLoan) {
		...
		try {
			...
			libraryBranch = bookLoan.getLibraryBranch();
			borrower = bookLoan.getBorrower();
			...
			prepareStatement.setDate(4, bookLoan.convertToDateViaSqlDate(bookLoan.getDateOut()));
